...
class NB:
    def __init__(self):
        self.beginTime = time()
        self.url = 'http://NB/newspaper/'
        self.agent = 'Mozilla/5.0 (Windows NT 10.0; WOW64; Trident/7.0; rv:11.0) like Gecko'
        try:
        ...
        self.opener.addheaders = self.noredirect_opener.addheaders = [
        ...
        try:
            ...
            sys.exit()
        self.fieldnames = ['Title', 'Author',
                           'Newspaper', 'Date', 'Page', 'Link']
        self.workers = 2
        self.interval = 1.2
    ...
    def getCookie(self):
        request = Request(self.url, method='HEAD', headers={
    ...
    def parse(self, page, newspaper, year):
        ...
        url = self.url + 'search.jsp?menuName={0}&year={1}&d-3995381-p={2}'.format(
            quote(newspaper, encoding='gbk'), year, page)
        html = self.openUrl(url)
        if not html:
            logger.error(
                'Failed to fetch %s Year:%s Page:%s content.', newspaper, year, page)
            return []
        ...
        for i in content:
            ...
            try:
                ...
                logger.error(
                    'A corrupted record was skipped.(Please check %s)', url)
        return result
    ...
    def DownloadPDF(self, record):
        ...
        page = record['Page']
        path = newspaper + '/' + \
            date.split('-')[0] + '/' + ''.join(date.split('-')[1:])
        if page == '未知':
            request = Request(link, method='HEAD')
            headers = self.noredirect_opener.open(request)
            filename = headers.info().get('Location').split(';')[0].split(
                '/')[-1].split('.')[0].lstrip('0').zfill(2) + '.pdf'
            sleep(random())
        else:
            ...
            for _ in range(3):
                try:
                    ...
                    if not file.info().get('Etag'):
                        dl_logger.info(
                            'The file could not be found - %s', fullpath)
                        error = 0
                        break
                    ...
                    dl_logger.error(
                        'Encounter error when downloading %s Waiting for retry...', fullpath)
                    sleep(randint(30, 60)+random())
                    error = 1
            if error == 1:
    ...
    def run(self):
        ...
        with open('NB.csv', 'w', encoding='utf-8-sig', newline='') as output_file:
            output = DictWriter(output_file, self.fieldnames,
                                extrasaction='ignore')
            output.writeheader()
            for n in self.newspaper:
                ...
                for y in year:
                    ...
                    with ThreadPoolExecutor(self.workers) as executor:
                        try:
                            return_list = list(executor.map(
                                partial(self.parse, newspaper=n, year=y), page))
                    ...
                    logger.info('Current downloading process: %s',
                                self.last_download)
                    logger.info(
                        'Current downloaded pdf files: %s', self.counter)
                    logger.info('Elapsed Time: %ss', self.elapsedTime())
            logger.info(
                'Newspaper info crawling finished. Total time: %ss', self.elapsedTime())
            logger.info(
                'Please wait for the pdf download process to complete...')
        self.DownloadExecutor.shutdown()
        dl_logger.info('Download complete. Total time: %ss',
                       self.elapsedTime())
